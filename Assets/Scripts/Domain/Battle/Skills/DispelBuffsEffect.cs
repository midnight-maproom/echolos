...
{
    // 敵対象の能力バフを解除する Effect。
    // 対象：AbilityModifier / EvasionModifier / OutgoingDamageModifier / IncomingDamageModifier
    //       / CriticalRateModifier / CounterDamageModifier のうち IsBuff=true なもの。
    // IsUndispellable=true（Persistent / Conditional オーラ）は剥がせない。
    public sealed class DispelBuffsEffect : IActionEffect
    {
        public void Apply(IActionContext context)
        {
            ...
            foreach (var target in targets)
            {
                ...
                {
                    if (IsBuffDispelTarget(e) && !e.IsUndispellable)
                        toRemove.Add(e);
                }
        ...
        private static bool IsBuffDispelTarget(IEffect e)
        {
            switch (e)
            {
                ...
                case HealReceivedModifier mod:    return mod.IsBuff;
